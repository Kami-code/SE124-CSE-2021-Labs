...
public:
     ...
     * */
    int execute(unsigned int shard_offset,
            unsigned int proc,
    ...
    bool check_shard_prepared(int tx_id);
...
public:
    ...
    std::map<int, std::mutex*> key_locks;
    std::map<int, int> hold_transaction;
    std::map<int, std::mutex*> entry_locks;

    std::mutex hold_tx_write_lock;
