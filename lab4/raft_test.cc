...
TEST_CASE(part1, re_election, "Election after network failure")
{
    ...
    printf("try to kill first leader");
    // 2. kill the leader
...
TEST_CASE(part2, fail_agree, "Fail Agreement")
{
    ...
    std::cout << "disables leader = " << leader << std::endl;
    ...
    std::cout << "enable_node = " << leader << std::endl;
...
TEST_CASE(part2, backup, "Leader backs up quickly over incorrect follower logs")
{
    ...
    group->append_new_command(value++, num_nodes);  //0
    ...
    int temp_term, temp_index;
    for (int i = 0; i < 2; i++)
        group->nodes[leader1]->new_command(list_command(value++), temp_term, temp_index); //1~50
    ...
    for (int i = 0; i < 2; i++)
        group->append_new_command(value++, 3);  //51~100
    ...
    for (int i = 0; i < 2; i++)
        group->nodes[leader2]->new_command(list_command(value++), temp_term, temp_index);   //101~150
	...
	// lots of successful commands to new group.
    for (int i = 0; i < 2; i++)
        group->append_new_command(value++, 3);  //151~200
    ...
    group->append_new_command(value++, num_nodes);  //201
    delete group;
}
...
//                ASSERT(cmd.res->value == ans_val, "Wrong value");
//            }
//>>>>>>> 97a18e12d4043f7a0a4b5abc54d9da9c7541300d
        }
        return;
        next_try:
            mssleep(200);
