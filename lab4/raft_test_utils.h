...
//            std::cout << "node " << i << " store_size = " << (int)state->store.size() << " log_idx = " << log_idx << std::endl;
...
int raft_group<state_machine, command>::append_new_command(int value, int expected_servers) {
    ...
    while (std::chrono::system_clock::now() < start + std::chrono::seconds(10)) {
        ...
        for (size_t i = 0; i < nodes.size(); i++) {
            ...
            if (i == nodes.size() - 1) {
//                std::cout << "no leaders found!" << std::endl;
            }
        }
        if (log_idx != -1) {
...
//                 std::cout <<"current value = " << value << "expected_servers = " << expected_servers << " number commited: " << committed_server << std::endl;
    ...
    ASSERT(0, "Cannot make agreement! " + std::to_string(value));
    return -1;
}
