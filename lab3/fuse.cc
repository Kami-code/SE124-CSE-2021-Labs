...
chfs_client::status getattr(chfs_client::inum inum, struct stat &st)
{
    ...
    }  else if (chfs->issymboliclink(inum)) {
        chfs_client::fileinfo info;
        ret = chfs->getfile(inum, info);
        if(ret != chfs_client::OK)
            return ret;
...
void fuseserver_read(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off, struct fuse_file_info *fi)
{
    ...
    printf("fuse read size = %d, off = %d\n", size, off);
    std::string buf;
    int ret = chfs->read(ino, size, off, buf);
    if (ret != chfs_client::OK) {
        printf("not good over\n");
        fuse_reply_err(req, ENOSYS);
        return;
    }
...
void fuseserver_write(fuse_req_t req, fuse_ino_t ino, const char *buf, size_t size, off_t off, struct fuse_file_info *fi)
{
    ...
    int ret = chfs->write(ino, size, off, buf, bytes_written);
    if (ret != chfs_client::OK) {
        fuse_reply_err(req, ENOSYS);
        return;
    }
    printf("successfully return in fuse.cc\n");
...
void fuseserver_mkdir(fuse_req_t req, fuse_ino_t parent, const char *name, mode_t mode) {
    ...
    int ret = chfs->mkdir(parent, name, mode, ino_out);
    if (ret == chfs_client::EXIST) {
        fuse_reply_err(req, EEXIST);
        return;
    }
    ...
    struct stat st;
    chfs_client::inum inum = ino_out; // req->in.h.nodeid;
...
//https://fossies.org/dox/fuse-3.10.5/structfuse__lowlevel__ops.html
void fuseserver_symlink(fuse_req_t req, const char *link, fuse_ino_t parent, const char *name) {
    int r;
    printf("in fuseserver_symlink inode = %d\n", parent);
    chfs_client::inum ino_out;
    if ((r = chfs->symlink(parent, link, name, ino_out)) != chfs_client::OK) {
        if (r == chfs_client::NOENT) {
            fuse_reply_err(req, ENOENT);
            return;
        } else {
    ...
    fuse_reply_entry(req, &e);
    printf("out fuseserver_symlink\n");
    return;
}

//http://blog.chinaunix.net/uid-24276740-id-3621499.html
void fuseserver_readlink(fuse_req_t req, fuse_ino_t ino) {
    int r;
    printf("in fuseserver_readlink inode = %d\n", ino);
    std::string link_content;
    if ((r = chfs->readlink(ino, link_content)) == chfs_client::OK) {
        fuse_reply_readlink(req, link_content.c_str());
    } else {
        if (r == chfs_client::NOENT) {
            fuse_reply_err(req, ENOENT);
        } else {
