...
#define IndexLengthInIndirectBlock 4
#define BytesPerDirectBlock 2
...
void inode_manager::free_inode(uint32_t inum) {
    ...
    if (tmp_inode_saver == NULL) {
        ...
        int done_size = free_blocks * BLOCK_SIZE;
        int indirect_block_size = 0;
        printf("a->size = %d\n", a.size);
        printf("free_blocks = %d\n", done_size);
        if (done_size < a.size) {
            if ((a.size - done_size) % BLOCK_SIZE == 0) {
                indirect_block_size = (a.size - done_size) / BLOCK_SIZE;
            }
            else {
                indirect_block_size = (a.size - done_size) / BLOCK_SIZE + 1;
            }
            printf("remain_block_size = %d \n", indirect_block_size);
        }
        if (tmp_inode_saver->blocks[NDIRECT] != 0) {
            ...
            for (int indirect_block_cursor = 0; indirect_block_cursor < indirect_block_size; indirect_block_cursor++) {
                unsigned char direct_block_in_bytes[BytesPerDirectBlock];
                memcpy(direct_block_in_bytes, indirect_block_data + indirect_block_cursor * BytesPerDirectBlock, BytesPerDirectBlock);
                direct_block_in_bytes[BytesPerDirectBlock] = '\0';
                blockid_t current_direct_block_number = ((direct_block_in_bytes[0] << 8) & (0x0000ff00)) + direct_block_in_bytes[1];
...
//                current_direct_block_number_string[IndexLengthInIndirectBlock] = '\0';
//                printf("current_direct_block_number_string = %s \n", current_direct_block_number_string);
...
//    printf("writer want to free_inode \n");
    free_inode(inum);
    verbose = true;
    //  if(verbose)
//    printf("开始写操作 inode- %d\n", inum);
    inode* current_inode = get_inode(inum);
    if (current_inode == NULL) {
//        printf("the inode has cleaned \n");
...
//    printf("\t size = %d buf = %s \n", size, buf);
//    printf("\t remaining_size = %d", remaining_size);
    while ((current_block_cursor < NDIRECT) && (remaining_size > 0)) {
//        printf("\t 当前写到了第%d个block, remaining_size = %d \n", current_block_cursor, remaining_size);
...
//      if (verbose) printf("\t create indirect number = %d \n", allocated_indirect_block_number);
        ...
        current_inode->blocks[NDIRECT] = allocated_indirect_block_number;
        while (remaining_size > 0) {
            ...
            unsigned char direct_block_in_bytes[BytesPerDirectBlock];
            direct_block_in_bytes[0] = (0x0000ff00 & allocated_direct_block_number) >> 8;
            direct_block_in_bytes[1] = (0x000000ff & allocated_direct_block_number);
            direct_block_in_bytes[BytesPerDirectBlock] = '\0';
            assert((current_indirect_block_cursor * BytesPerDirectBlock + BytesPerDirectBlock) < BLOCK_SIZE);
            memcpy(indirect_block_data + current_indirect_block_cursor * BytesPerDirectBlock, direct_block_in_bytes, BytesPerDirectBlock);

            unsigned char unmarsal_direct_block_in_bytes[BytesPerDirectBlock];
            memcpy(unmarsal_direct_block_in_bytes, indirect_block_data + current_indirect_block_cursor * BytesPerDirectBlock, BytesPerDirectBlock);
            blockid_t unmarsal_direct_block_number = ((direct_block_in_bytes[0] << 8) & (0x0000ff00)) + direct_block_in_bytes[1];
            assert(unmarsal_direct_block_number == allocated_direct_block_number);

            current_indirect_block_cursor ++;
        }

        unsigned char last_end_marker[BytesPerDirectBlock];
        last_end_marker[0] = 0xff;
        last_end_marker[1] = 0xff;
        last_end_marker[BytesPerDirectBlock] = '\0';
        assert((current_indirect_block_cursor * BytesPerDirectBlock + BytesPerDirectBlock) < BLOCK_SIZE);
        memcpy(indirect_block_data + current_indirect_block_cursor * BytesPerDirectBlock, last_end_marker, BytesPerDirectBlock);
